...
ASSET_URL=$(curl -fsSL "$RELEASE_URL" | grep -o "\"browser_download_url\": \"[^\"]*${PLATFORM}[^\"]*\\.${EXT}\"" | head -1 | sed 's/.*: "\(.*\)"/\1/')

if [ -z "$ASSET_URL" ]; then
    echo "Error: No ${PLATFORM} binary found in latest release."
